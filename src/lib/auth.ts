...
import { runSingleFlight } from './requestCache';
...
async function fetchCurrentAuthUserId(): Promise<string> {
  return runSingleFlight('auth:getUser', async () => {
    const {
      data: { user },
      error,
...
export async function requireAuthenticatedUserId(expectedUserId?: string): Promise<string> {
  const currentUserId = await fetchCurrentAuthUserId();
