...
export default function MemberStatsScreen() {
  ...
  const heroData = useMemo(() => {
    ...
    goals.forEach((ug) => {
      ...
      goalWeeks.forEach((gw) => {
        ...
        totalDoneG += checkins
          .filter(
            (c) =>
              c.goal_id === ug.goal_id &&
              c.date >= gw.s.format('YYYY-MM-DD') &&
              c.date <= effE.format('YYYY-MM-DD'),
          )
    ...
    const avgRate =
      allGoalRates.length > 0
        ? Math.round(allGoalRates.reduce((a, b) => a + b, 0) / allGoalRates.length)
        : 0;
    return {
      avgRate,
      altitude: Math.round(avgRate * 100),
      totalDone,
      totalPass: totalPass + autoPass,
    };
  ...
  const weeklyPace = useMemo(() => {
    ...
    const paceGoals: WeeklyPaceGoal[] = goals
      ...
      .map((ug) => {
        ...
        const done = checkins
          .filter((c) => c.goal_id === ug.goal_id && c.date >= weekMonStr && c.date <= todayStr)
          .filter(isDoneCheckin).length;
        return {
          goalId: ug.goal_id,
    ...
    return {
      goals: paceGoals,
      overallRate,
      weekLabel: `${weekMon.format('M/D')}~${weekMon.add(6, 'day').format('M/D')}`,
    };
  }, [goals, checkins, allGoalMap]);
  ...
  const trendData = useMemo(() => {
    ...
    ranges.forEach((wr, idx) => {
      ...
      goals.forEach((ug) => {
        ...
        const wCk = checkins.filter(
          (c) =>
            c.goal_id === ug.goal_id &&
            c.date >= gS.format('YYYY-MM-DD') &&
            c.date <= gE.format('YYYY-MM-DD'),
        );
  ...
  const goalDetails = useMemo(() => {
    const calcStats = (ugList: typeof goals): GoalStat[] =>
      ugList.map((ug) => {
        let done = 0,
          pass = 0,
          fail = 0;
      ...
      .map((ug) => {
        ...
        const dayEntries = activeDays.map((d) => {
          ...
          if (d < today)
            return {
              date: dateLabel,
              status: isWeekly ? ('auto_pass' as const) : ('fail' as const),
            };
        ...
        return {
          ...
          name,
          isWeekly,
          target: isWeekly ? ug.target_count || 1 : activeDays.length,
          doneCount: dayEntries.filter((e) => e.status === 'done').length,
          dayEntries,
        };
      });
      ...
      <ScrollView style={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.heroCard}>
          ...
          <Text style={s.heroTitle}>
            {dayjs(`${yearMonth}-01`).format('M')}월, 해발 {heroData.altitude.toLocaleString()}m{' '}
            {heroData.avgRate >= 100
              ? '정상 등극!'
              : heroData.avgRate >= 80
                ? '돌파!'
                : heroData.avgRate >= 50
                ...
                return (
                  <View key={g.goalId} style={s.paceRow}>
                    <View style={s.paceInfo}>
                      ...
                      <Text style={s.paceName} numberOfLines={1}>
                        {g.name}
                      </Text>
                    </View>
                    <View style={s.paceProgress}>
                      <ProgressBar
                        ...
                        height={6}
                        color={color}
                        maxRate={Math.max(100, g.rate)}
                      />
                      <Text style={[s.paceVal, { color }]}>
                        {g.done}/{g.target} {g.rate > 100 ? `(${g.rate}%)` : ''}
                      </Text>
                    </View>
                  </View>
                );
              ...
              <View style={s.trendLegend}>
                {trendData.map((w, idx) => (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.7}
                  >
                    <Text style={s.trendLegendLabel}>
                      {w.label} <Text style={s.trendLegendDays}>({w.days}일)</Text>
                    </Text>
                    <Text style={s.trendLegendRange}>{w.range}</Text>
                    <Text style={s.trendLegendVal}>
                      인증 {w.doneCount} · 패스 {w.passCount}
                    </Text>
                  </TouchableOpacity>
                ))}
              </View>
                  ...
                  {weekDetail.map((g) => (
                    <View key={g.goalId} style={s.weekDetailGoal}>
                      <View style={s.weekDetailGoalHead}>
                        <Text style={s.weekDetailGoalName} numberOfLines={1}>
                          {g.name}
                        </Text>
                          ...
                          <View key={entry.date} style={s.weekDetailDay}>
                            ...
                            <View
                              style={[
                                ...
                                (entry.status === 'pass' || entry.status === 'auto_pass') &&
                                  s.weekDetailBadgePass,
            ...
            <View style={s.spotlightRow}>
              <View style={[s.spotlightCard, { backgroundColor: 'rgba(74,222,128,0.08)' }]}>
                ...
                <Text style={s.spotlightName} numberOfLines={1}>
                  {goalDetails.best.name}
                </Text>
                <Text style={s.spotlightStat}>
                  완료 {goalDetails.best.done}회 · {goalDetails.best.rate}%
                </Text>
              </View>
              {goalDetails.worst ? (
                ...
                <View key={gs.goalId} style={s.goalItem}>
                  <View style={s.goalItemHead}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, flex: 1 }}>
                      ...
                      <Text style={s.goalItemName} numberOfLines={1}>
                        {gs.name}
                      </Text>
                    </View>
                  ...
                  {gs.startDate && (
                    <Text style={s.goalStartDate}>
                      {dayjs(gs.startDate).format('M/D')}부터 시작
                    </Text>
                  )}
                  <ProgressBar
                    rate={gs.rate}
                  ...
                  <View style={s.goalChipRow}>
                    ...
                    {gs.pass + gs.fail > 0 && (
                      <View style={[s.goalChip, { backgroundColor: 'rgba(239,68,68,0.08)' }]}>
                        <Text style={[s.goalChipText, { color: '#b91c1c' }]}>
                          패스 {gs.pass + gs.fail}
                        </Text>
                      </View>
                    )}
