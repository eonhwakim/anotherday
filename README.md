...
| **State Management** | ![Zustand](https://img.shields.io/badge/Zustand-orange?style=flat&logo=react&logoColor=white) ![TanStack Query](https://img.shields.io/badge/TanStack_Query-FF4154?style=flat&logo=reactquery&logoColor=white) |
...
### 2. React Query + Zustand 역할 분리
서버 상태와 UI 상태의 성격이 다르기 때문에 현재는 **TanStack Query**와 **Zustand**를 함께 사용합니다.
- **React Query:** 팀, 목표, 체크인, 통계처럼 Supabase에서 읽고 쓰는 서버 상태를 캐시하고 무효화합니다.
- **Zustand:** 인증 세션, 현재 선택 팀, 화면 내부 UI 상태처럼 앱 로컬 상태만 담당합니다.
- **운영 규칙:** query key와 invalidate 원칙은 [docs/react-query-guidelines.md](docs/react-query-guidelines.md)에서 관리합니다.
...
```typescript
// src/queries/statsMutations.ts 예시
useMutation({
...
├── queries/         # React Query hooks / mutations / query keys
├── stores/          # Zustand 로컬 UI 상태
